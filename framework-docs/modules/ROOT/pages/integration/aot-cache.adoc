...
application. It is possible to create this cache during a
...
NOTE: However, it is important to note that AOT cache stores, among other heuristics, 
method profiling information. It implies, that in order to get the most efficient AOT cache
possible it is generally recommended for your service to experience a portion of a real workload,
so that the generated AOT cache stores the profiling information that is closest to production
conditions.
